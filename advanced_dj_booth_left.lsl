...
startSpin()
{
    llSetLinkPrimitiveParamsFast(14,[PRIM_OMEGA,<0,0,-1>,10,0.01]);
}

stopSpin()
{
    llSetLinkPrimitiveParamsFast(14,[PRIM_OMEGA,<0,0,0>,0,0]);
}
...
stopPlayback()
{
    ...
    llRegionSay(playchannelLeft,(key)"0d0f06a3-daf3-d572-b19e-7622a114e40c");
    STARTED=FALSE;
    PLAYING=FALSE;
    llStopSound();
}
...
toggleVolume()
{
    ...
    {
        VOLUME=0;
        llSetLinkPrimitiveParamsFast(37,[PRIM_COLOR,ALL_SIDES,<1,0,0>,1]);
    }
    else
    {                        startSpin();
        ...
        VOLUME=1;
        llSetLinkPrimitiveParamsFast(37,[PRIM_COLOR,ALL_SIDES,<0,1,0>,1]);
    }
...
setArtwork(key artwork)
{
    ...
    llSetLinkPrimitiveParamsFast(14,[PRIM_TEXTURE,0,artwork,<1,1,0>,<0,0,0>,0]);
}
    ...
    link_message(integer sender_num, integer num, string list_argument, key id)
    {
        if(currentAlbum!="")
        {
            ...
            else
            {
                ...
                else if(num==38|num==27|num==26|num==25)
                {
                    //Toggle Volume Button
                    toggleVolume();
                }
                else if(num==34|num==33)
                {
                    //Eject Button
                    purgeInventory();
                    stopPlayback();
